...
void StartBenchmark(ChessEngine engine, int depth) {
    ...
    engine.maxTime = 100000000;
    engine.Trasposition.clear();

    auto start = GetTimeMs();
    engine.startTime = start;
    Move bestmove = engine.BestMove(depth, board);
    auto end = GetTimeMs();
