...
std::uint64_t setOccupancy(int square, int index, std::uint64_t mask) {
...
void Bitboard::generateBishopAttacks() {
    for (int square = 0; square < 64; ++square) {
        ...
        for (int i = 0; i < numOccupancies; ++i) {
            std::uint64_t magic = (setOccupancy(square,i,mask) * bishopMagic[square]) >> (64 - numbits);
...
void Bitboard::generateRookAttacks() {
    ...
    for (int square = 0; square < 64; ++square) {
        ...
        for (int i = 0; i < numOccupancies; ++i) {
            std::uint64_t magic = (setOccupancy(square, i, mask) * rookMagic[square]) >> (64 - numbits);
